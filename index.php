        ...
        'site'         => function ($event, $app) {
            $app->on('view.content', function ($event, $scripts) use ($app) {
                ...
                if ((!$this->config['nodes']
                    ...
                    $params['resetOnEnd']    = (!empty($config['reset'])
                        ? $config['reset'] : true);
                    $params['loopCheckTime'] = (!empty($config['checktime'])
                        ? $config['checktime'] : 0);
                    $params['loopMaxNumber'] = (!empty($config['loopnumber'])
                        ? $config['loopnumber'] : 0);
                    $params['baitClass']     = (!empty($config['baitclass'])
                        ? $config['baitclass'] : '');
                    $params['baitStyle']     = (!empty($config['baitstyle'])
                        ? $config['baitstyle'] : '');
                    $params['debug']         = (!empty($config['debug'])
                        ? $config['debug'] : false);
                        ...
                        $script
                            .= "
								function adBlockDetected() {
									...
									var parser = new DOMParser();
									var doc = parser.parseFromString( s, 'text/html' );
									if(!getCookie('detectadblockershown')){
									setCookie('detectadblockershown', 'true', 365);
                    ...
                    $app['scripts']->add('blockadblock',
                        'spqr/detectadblocker:app/assets/blockadblock/blockadblock.js');
                    
                    $app['scripts']->add('detectadblocker', $script, [],
                        'string');
                }
            });
        },
        'view.scripts' => function ($event, $scripts) use ($app) {
            $scripts->register('detectadblocker-settings',
